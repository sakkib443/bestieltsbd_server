...
// Purchase a mock test with payment
const purchaseMock = async (
    ...
    const payment = await Payment.create({
        ...
        status: "completed", // For now, mark as completed (manual verification)
        transactionId,
    ...
    // Update payment with purchase ID
    ...
    if (user) {
        const studentRecord = await Student.create({
            ...
            assignedSets: {
                fullSets: [{
                    label: pkg.title,
                    listeningSetNumber: pkg.examSets.listeningSetNumber,
                    readingSetNumber: pkg.examSets.readingSetNumber,
                    writingSetNumber: pkg.examSets.writingSetNumber,
                }]
            },
            examStatus: "not-started",
            isActive: true,
